  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-5">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[12px] text-ink-muted">
            Configure per-tier feature flags. Changes take effect immediately for new
            documents — existing documents are not retro-stamped.
          </p>
        </div>
        <button
        ...
        >
          <Plus className="w-3.5 h-3.5" /> {showNew ? 'Close' : 'New tier'}
        </button>
      </header>
          ...
          <table className="w-full text-[13px]">
            <thead className="bg-bg-subtle text-left text-[10px] uppercase tracking-wider text-ink-faint">
              <tr>
                ...
                <th className="py-2 px-3 w-44 text-center">Branding watermark</th>
                <th className="py-2 px-3 w-24">Sort</th>
              ...
              {(tiersQuery.data ?? []).map((row) => (
                <Row
                  ...
                  onDelete={() => {
                    if (
                      confirm(
                        `Delete tier "${row.code}"? Built-in tiers can't be deleted; disable instead.`,
                      )
                    ) {
      ...
      <p className="text-[11px] text-ink-faint">
        The "Branding watermark" toggle controls whether documents generated by users on
        that tier get the platform watermark stamped onto the PDF. Other tier-level flags
        can be added later by extending the <code>TierConfigModel.features</code> JSON column.
      </p>
    </div>
  );
}
...
function NewTierForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    ...
    >
      <div>
        <label className="block text-[11px] text-ink-muted mb-1">Code</label>
        <input
          value={code}
      ...
      <div>
        <label className="block text-[11px] text-ink-muted mb-1">Sort</label>
        <input
          type="number"
          value={sortOrder}
          onChange={(e) => setSortOrder(Number(e.target.value))}
