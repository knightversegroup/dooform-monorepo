...
export class GetTemplateThumbnailUseCase implements UseCase<GetTemplateByIdDto, { buffer: Buffer; filename: string }> {
  ...
  async execute(dto: GetTemplateByIdDto): Promise<Result<{ buffer: Buffer; filename: string }>> {
    ...
    if (!template.filePathThumbnail) throw new InvalidOperationException('No thumbnail available')

    const buffer = await this.storageService.read(template.filePathThumbnail)
    return { buffer, filename: 'thumbnail.png' } as any
  }
}
