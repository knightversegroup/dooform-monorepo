...
export class DeleteDocumentTypeUseCase implements UseCase<GetTemplateByIdDto, { success: boolean }> {
  constructor(
  ...
  async execute(dto: GetTemplateByIdDto): Promise<Result<{ success: boolean }>> {
    const exists = await this.documentTypeRepository.exists(dto.id)
    if (!exists) {
