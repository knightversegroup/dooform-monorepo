...
export class GetTemplatePdfPreviewUseCase implements UseCase<GetTemplateByIdDto, { buffer: Buffer; filename: string }> {
  constructor(
  ...
  async execute(dto: GetTemplateByIdDto): Promise<Result<{ buffer: Buffer; filename: string }>> {
    ...
    if (!template.filePathPDF) throw new InvalidOperationException('No PDF preview available')

    const buffer = await this.storageService.read(template.filePathPDF)
    return { buffer, filename: 'preview.pdf' } as any
  }
}
