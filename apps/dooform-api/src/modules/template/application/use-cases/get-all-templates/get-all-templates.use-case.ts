...
export class GetAllTemplatesUseCase implements UseCase<GetAllTemplatesDto, GetAllTemplatesResult> {
  ...
  async execute(dto: GetAllTemplatesDto, userTier?: string): Promise<Result<GetAllTemplatesResult>> {
    ...
    const isAdmin = canReadCrossOrg || dto.callerRole === 'ORG_ADMIN'

    // Grouped query: return templates grouped by document type
    if (dto.grouped) {
      return this.executeGrouped(dto, userTier, isAdmin, canReadCrossOrg)
    }
  ...
  private async executeGrouped(
    dto: GetAllTemplatesDto,
    userTier?: string,
    _isAdmin = false,
    canReadCrossOrg = false,
  ): Promise<Result<any>> {
    ...
    //  - `templates:read-cross-org`: no further filter.
