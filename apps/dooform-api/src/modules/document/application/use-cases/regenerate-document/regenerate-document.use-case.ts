...
export class RegenerateDocumentUseCase implements UseCase<RegenerateDocumentDto, RegenerateDocumentResult> {
  ...
  async execute(dto: RegenerateDocumentDto): Promise<Result<RegenerateDocumentResult>> {
    ...
    return {
      ...
      download_url: `/api/v1/documents/${saved.id}/download`,
      download_pdf_url: `/api/v1/documents/${saved.id}/download?format=pdf`,
      document: {
        id: saved.id,
        template_id: props.templateId,
        user_id: props.userId,
        filename: props.filename,
        file_path_docx: props.filePathDocx,
        file_path_pdf: props.filePathPdf,
