...
interface PresetCardProps {
  ...
  onEdit: (p: WatermarkPreset) => void;
  onDelete: (p: WatermarkPreset) => void;
  deleting: boolean;
}

function PresetCard({ preset, onEdit, onDelete, deleting }: PresetCardProps) {
  const logoUrl = useWatermarkLogoUrl(preset.id, preset.logo_path ?? null);
  return (
    <div className="border border-gray-200 rounded-lg bg-white overflow-hidden flex flex-col">
      <div className="bg-[#f6f6f6] flex items-center justify-center p-6 h-40">
        <WatermarkPreview config={preset.config} logoUrl={logoUrl} width={200} height={120} />
      </div>
      <div className="flex-1 p-4 flex flex-col gap-1">
        <h3 className="font-semibold text-gray-900 truncate" title={preset.name}>
          {preset.name}
        </h3>
        <p className="text-xs text-gray-500">
          {preset.config.lines.length} บรรทัด
          {preset.logo_path ? " • มีโลโก้" : ""}
        </p>
      </div>
